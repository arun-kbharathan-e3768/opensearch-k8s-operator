...
func (r *TLSReconciler) handleTransport() error {
	...
	// Update transport cert expiry status
	r.updateTransportCertExpiryStatus()
...
func (r *TLSReconciler) handleHttp() error {
	...
	// Update HTTP cert expiry status
	r.updateHttpCertExpiryStatus()
...
func (r *TLSReconciler) updateTransportCertExpiryStatus() {
	clusterName := r.instance.Name
	namespace := r.instance.Namespace
	secretName := clusterName + "-transport-cert"

	secret, err := r.client.GetSecret(secretName, namespace)
	if err != nil {
		return
	}
	...
	validator, err := tls.NewCertValidater(certData)
	if err != nil {
		return
	}
	expiryTime := validator.ExpiryDate()
	key := client.ObjectKey{Name: clusterName, Namespace: namespace}
	_ = r.client.UpdateOpenSearchClusterStatus(key, func(cluster *opensearchv1.OpenSearchCluster) {
		cluster.Status.TransportCertificateExpiry = metav1.NewTime(expiryTime)
	})
}

func (r *TLSReconciler) updateHttpCertExpiryStatus() {
	clusterName := r.instance.Name
	namespace := r.instance.Namespace
